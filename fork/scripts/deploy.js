...
async function getPool(tokenA, tokenB, feeAmount, provider){
    ...
    console.log("Generated Pool Address:", Pool.getAddress(token0, token1, feeAmount));
    console.log("ABI Loaded:", Array.isArray(IUniswapV3Pool.abi)); // Should print: true
    console.log("ABI Length:", IUniswapV3Pool.abi.length); // Should print: >0
    console.log("Provider Check:", provider ? "Valid" : "Invalid");
...
async function main() {
    ...
    console.log("usdcBalance:",hardhat.ethers.utils.formatUnits(usdcBalance,18));
